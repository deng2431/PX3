...
#$pItem->();
     ...
     $pItem->setCountry(eBay::API::XML::DataType::Enum::CountryCodeType::AU);
     $pItem->setCurrency(eBay::API::XML::DataType::Enum::CurrencyCodeType::AUD);
	...
	# $pItem->setPrimaryCategory()->setCategoryID(377);
	 ...
	 my $sItemfee = $pCall->getItemID()->getValue();
	 ...
	 if($checkAck eq "Success"){
		...
		print "Item has successfuly added to ebay with the Item ID:  $sItemId \n".;
		print "Item Fee: $sItemfee \n"
